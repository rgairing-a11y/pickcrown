...
export const EVENT_TYPES = {
  ...
  // NFL Playoff style (reseeding, advancement picks) - LEGACY
  advancement: {
    name: 'Advancement',
  ...
  // NFL Bracket - True bracket with reseeding after Wildcard
  // Uses bracket_picks table like NCAA, but Divisional matchups are generated
  // based on actual Wildcard winners (server) or user picks (client projection)
  nfl_bracket: {
    name: 'NFL Bracket',
    archetype: 'bracket',
    scoringFunction: 'calculate_standings',  // Standard bracket scoring
    pickTable: 'bracket_picks',
    pickColumn: 'picked_team_id',
    hasMatchups: true,
    hasCategories: false,
    hasTeamEliminations: false,  // Uses matchups, not eliminations
    usesReseeding: true,  // Divisional round is reseeded
    reseedingConfig: {
      // Round order after which reseeding occurs
      reseedAfterRound: 1,  // After Wildcard (round_order=1)
      // How teams are paired after reseeding
      pairingStrategy: 'highest_vs_lowest',  // #1 vs lowest, next vs next
    },
    features: {
      scenarioSimulator: true,
      pathToVictory: true,
...
// Helper to check if event uses NFL-style reseeding
export function usesNFLReseeding(event) {
  const config = getEventTypeConfig(event)
  return config.usesReseeding === true
}

// Helper to get reseeding config for an event
export function getReseedingConfig(event) {
  const config = getEventTypeConfig(event)
  return config.reseedingConfig || null
}
