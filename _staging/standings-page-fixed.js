...
export default async function StandingsPage({ params }) {
  ...
  if (isLocked) {
    ...
    if (allMatchups && allMatchups.length > 0) {
      ...
      if (entries && entries.length > 0) {
        ...
        // Show path to victory whether games remain or not
        const eventComplete = undecidedMatchups.length === 0
          ...
          sortedStandings.forEach((entry, idx) => {
            ...
            if (idx === 0) {
              ...
              if (secondPlace) {
                ...
                let leaderMessage
                let leaderStatus = 'leading'
                
                if (eventComplete) {
                  leaderMessage = leadOverSecond === 0 
                    ? `🏆 Champion! (tied with ${leadOverSecond} pts)`
                    : `🏆 Champion! Won by ${leadOverSecond} pts.`
                  leaderStatus = 'clinched'
                } else if (hasClinched) {
              ...
              let challengerStatus
              
              if (eventComplete) {
                challengerMessage = gap === 0 
                  ? `🥈 Tied for the win!`
                  : `Finished ${gap} pts behind the winner.`
                challengerStatus = gap === 0 ? 'tied' : 'finished'
              } else {
                challengerStatus = !canCatchUp ? 'eliminated' : gap === 0 ? 'tied' : 'chasing'
                challengerMessage = !canCatchUp 
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
      ...
      {/* Path to Victory / Final Results Section */}
      {isLocked && pathToVictory.length > 0 && (
        <div style={{ marginTop: 48 }}>
          <h2 style={{ fontSize: '20px', marginBottom: 8 }}>
            {remainingMatchups > 0 ? '🛤️ Path to Victory' : '🏁 Final Results'}
          </h2>
          {remainingMatchups > 0 ? (
            ...
            {pathToVictory.map((pv, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '12px 16px',
                    background: pv.status === 'eliminated' ? '#fef2f2' : 
                               pv.status === 'finished' ? '#f9fafb' : '#fff',
                    borderRadius: 8,
                ...
                >
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ 
                        fontWeight: 600, 
                        fontSize: 15,
                        color: pv.status === 'eliminated' || pv.status === 'finished' ? '#9ca3af' : '#374151'
                      }}>
                        {pv.entry_name}
                      </span>
                      {pv.status === 'clinched' && (
                        <span style={{ fontSize: 12, color: '#16a34a', fontWeight: 600 }}>🏆 Winner</span>
                      )}
                      {pv.status === 'leading' && (
                      ...
                      {pv.status === 'tied' && (
                        <span style={{ fontSize: 12, color: '#6366f1', fontWeight: 600 }}>🤝 Tied</span>
                      )}
                    </div>
                    <div style={{ 
                      fontSize: 14, 
                      ...
                      {remainingMatchups > 0 ? `${currentPts} → ${pv.maxTotal} pts` : `${currentPts} pts`}
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    {/* Max possible (lighter) - only show when games remain */}
                    {remainingMatchups > 0 && (
                      <div style={{
                        position: 'absolute',
                        height: '100%',
                    ...
                    <div style={{
                      position: 'absolute',
                      height: '100%',
                      width: remainingMatchups > 0 ? `${currentWidth}%` : `${(currentPts / (pathToVictory[0]?.maxTotal || currentPts)) * 100}%`,
                      background: pv.status === 'clinched' ? '#22c55e' :
                                 pv.status === 'leading' ? '#eab308' :
                                 pv.status === 'eliminated' ? '#9ca3af' :
                                 pv.status === 'finished' ? '#6b7280' :
                                 '#3b82f6',
                      borderRadius: 4
                    }} />
                  </div>
