-- PickCrown: Complete Database Fix - FINAL WORKING VERSION
-- Schema confirmed:
-- - category_picks.option_id ✓
-- - bracket_picks has NO points column (calculate dynamically) ✓
-- Date: 2026-01-08
...
BEGIN
  ...
  WITH season_points AS (
    SELECT 
      ...
      -- Calculate bracket points by checking if picked team won
      COALESCE(SUM(
        CASE 
          WHEN bp.picked_team_id = m.winner_id AND m.winner_id IS NOT NULL 
          THEN r.point_value
          ELSE 0
        END
      ), 0) as bracket_points,
    ...
    LEFT JOIN matchups m ON bp.matchup_id = m.id
    LEFT JOIN rounds r ON m.round_id = r.id
    LEFT JOIN category_picks cp ON cp.pool_entry_id = pe.id
    LEFT JOIN category_options co ON cp.option_id = co.id
...
SELECT '✓ CSV import ready' AS csv_status;
SELECT '✓ Season standings ready (calculates bracket + category points)' AS standings_status;
