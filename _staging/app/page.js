...
export default function HomePage() {
  ...
  async function loadUserData(userEmail) {
    ...
    // Get user's entries with full context
  ...
  const isHappeningNow = (event) => {
    ...
    // Locked = past start time, not completed
    // In Progress = locked but results not all in
  ...
  const processEntries = () => {
    const happeningNow = []
    const seasonMap = new Map() // seasonId -> { season, entries[] }
    const standalone = []
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 24, justifyContent: 'center', fontSize: 14, flexWrap: 'wrap' }}>
          ...
          <Link href="/find-my-pools" style={{ color: '#3b82f6' }}>
            🔍 Find My Pools
          </Link>
