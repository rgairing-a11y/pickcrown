...
const ROUND_PRESETS = {
  ...
  cfb: {
    ...
    rounds: [
      ...
      { name: 'National Championship', round_order: 4, points: 8 },
    ]
  },
  nhl: {
    name: 'NHL Playoffs',
    rounds: [
      { name: 'First Round', round_order: 1, points: 1 },
      { name: 'Second Round', round_order: 2, points: 2 },
      { name: 'Conference Finals', round_order: 3, points: 4 },
      { name: 'Stanley Cup Final', round_order: 4, points: 8 },
    ]
  },
  mlb: {
    name: 'MLB Playoffs',
    rounds: [
      { name: 'Wild Card', round_order: 1, points: 1 },
      { name: 'Division Series', round_order: 2, points: 2 },
      { name: 'Championship Series', round_order: 3, points: 4 },
      { name: 'World Series', round_order: 4, points: 8 },
    ]
  },
  worldcup: {
  ...
  simple_8: {
    name: 'Simple 8-Team',
    rounds: [
      { name: 'Quarterfinals', round_order: 1, points: 1 },
      { name: 'Semifinals', round_order: 2, points: 2 },
      { name: 'Final', round_order: 3, points: 4 },
    ]
  }
}
...
export default function RoundsAdminPage({ params }) {
  ...
  async function handleDeleteRound(roundId) {
    if (!confirm('Delete this round? All matchups in this round will also be deleted.')) return

    const res = await fetch(`/api/rounds?id=${roundId}`, { method: 'DELETE' })
    const data = await res.json()
  ...
  async function handleApplyPreset(presetKey) {
    const preset = ROUND_PRESETS[presetKey]
    if (!preset) return
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, padding: 16, background: '#f3f4f6', borderRadius: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ fontWeight: 600, color: '#3b82f6' }}>2. Rounds</span>
        <span style={{ color: '#9ca3af' }}>→</span>
      ...
      <div style={{ padding: 20, background: '#f9fafb', borderRadius: 8, marginBottom: 24 }}>
        ...
        <form onSubmit={handleAddRound} style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <input
            ...
            required
            style={{ flex: 2, minWidth: 150, padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: '14px' }}
          />
          <input
            type="number"
            value={roundOrder}
          ...
          <button
            type="submit"
            disabled={saving || !roundName.trim() || !roundOrder || !points}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {Object.entries(ROUND_PRESETS).map(([key, preset]) => (
              <button
                ...
                disabled={saving}
                style={{ padding: '8px 14px', background: 'white', color: '#0369a1', border: '1px solid #0369a1', borderRadius: 6, cursor: 'pointer', fontSize: '13px' }}
              >
                {preset.name}
              </button>
            ))}
              ...
              {rounds.map(round => (
                <tr key={round.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ padding: 12, textAlign: 'center' }}>
                    <span style={{ display: 'inline-block', padding: '4px 12px', background: '#dcfce7', color: '#166534', borderRadius: 12, fontWeight: 600 }}>
                  ...
                  <td style={{ padding: 12, textAlign: 'right' }}>
                    <button
                      ...
                      style={{ padding: '4px 8px', background: 'transparent', color: '#dc2626', border: '1px solid #dc2626', borderRadius: 4, cursor: 'pointer', fontSize: '12px' }}
                    >
                      Delete
                    </button>
                  </td>
